...
with model:
    ...
    out_pop = nengo.Ensemble(N, dimensions=2, radius = 1, label="Output")

    input1 = nengo.Node(piecewise({0: [1, 0], 0.1: [0, 0]}), label="input")
    
    nengo.Connection(input1, osc_pop, synapse=tau) 
    nengo.Connection(osc_pop, osc_pop, transform = [[1,-w],[w,1]], synapse = 0.1)
    ...
    nengo.Connection(draw_pop, out_pop, function=drawAx, transform=[[1],[0]], synapse=tau, eval_points=samples(5000))
    nengo.Connection(draw_pop, out_pop, function=drawAy, transform=[[0],[1]], synapse=tau, eval_points=samples(5000))

    out_probe = nengo.Probe(out_pop, synapse=0.01)
    draw_spikes = nengo.Probe(draw_pop, 'spikes')
    osc_probe = nengo.Probe(osc_pop, synapse=0.01)


import nengo_gui
gui = nengo_gui.Config()
gui[model].scale = 2.3586206896551727
gui[model].offset = -32.75862068965523,86.4112234181371
gui[draw_pop].pos = 196.623, 67.880
gui[draw_pop].scale = 1.000
gui[osc_pop].pos = 131.771, 68.929
gui[osc_pop].scale = 1.000
gui[out_pop].pos = 254.645, 67.446
gui[out_pop].scale = 1.000
